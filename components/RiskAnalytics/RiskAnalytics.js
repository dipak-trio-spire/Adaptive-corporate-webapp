...
import { FetchInteractiveRisk } from "@/pages/api/FetchInteractiveRisk";
import PageLoader from '@/components/PageLoader/PageLoader';

export default function RiskAnalytics(props) {
  ...
  const changeLevel = (percent) => {
    ...
    if (RiskData.length !== 0) {
      GenerateResponse(RiskData, percent, periodSelected);
    }
  };
  ...
  const changeDuration = (period) => {
    ...
    if (RiskData.length !== 0) {
      GenerateResponse(RiskData, Level, period);
    }
  };
  ...
  async function GenerateResponse(risk_data, level, period) {
    console.log("risk_data",risk_data)

    const Response = Object.keys(risk_data)
      .filter(key => responseKeys.includes(key))
      .reduce((obj, key) => {
        obj[key] = risk_data[key];
        return obj;
      }, {});

    if (risk_data[period] !== undefined) {
      Response['results'] = risk_data[period][level];
    }
  ...
  const SubmitPortfolioShield = (e) => {
    ...
    if (validFlag) {
      if (pli.length > 0) {
        ...
        setApiRequest(raw);
        setBdOpen(true);
        FetchInteractiveRisk(raw).then((risk_data) => {
          if (risk_data !== false && risk_data !== 0 && risk_data !== undefined) {
            setRiskData(risk_data);
            GenerateResponse(risk_data, Level, periodSelected);
            setBdOpen(false);
            TagManager.dataLayer({
              dataLayer: {
                event: 'Interactive API Used',
                apiName: 'Risk Analytics'
              },
            });
          }
